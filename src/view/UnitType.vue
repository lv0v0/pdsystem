...
<!--      <el-upload
    ...
    <el-table :data="tableData" @selection-change="handleSelectionChange" border stripe>
      ...
      <el-table-column prop="brandId" label="设备品牌" width="120">
      </el-table-column>
    ...
    <el-dialog title="设备信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="70px">
        ...
        <el-form-item label="设备型号">
          <el-input v-model="form.utype" :step="2" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods:{
    //将事件绑定到方法中
    load(){
      this.request.get("/unittype/page", {
        params:{
    ...
    //新增/编辑用户信息
    save(){
      this.request.post("/unittype", this.form).then(res => {
        if(res.code === '200'){
    ...
    //删除信息
    del(id){
      this.request.delete("/unittype/" + id).then(res =>{
        if(res.code === '200'){
